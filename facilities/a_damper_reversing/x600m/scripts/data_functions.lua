...
function createBlowerStartupDB()
  ...
  if not luatest_running then print("Created Blower Startup Database Successful") end
...
function updateEPATempAverages(zn_id,probe_ids)
  ...
  if COUNT_PFRP_IN_WARMUP_ZONE and reg['zone'..zn_id..'regime'] == 2 then
    can_count = true
  end
  if can_count then
    local avg_temp,
    ...
    if curr_pfrp_time < pfrp_duration then
      if check_temp < PFRP_TEMP then
        reg['zone'..zn_id..'pfrptime'] = 0
      else
        reg['zone'..zn_id..'pfrptime'] = curr_pfrp_time + SETTINGS['DataLoggingRate']
      end
    end
  end
end
