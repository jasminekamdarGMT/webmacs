function initValues()
  ...
  ZONE_LOG_COLUMNS = {'Temperature A', 'Damper', 'Blower Speed', 'Duct Pressure'}
...
function defaultSettings()
  ...
  settings["FacilityName"] = "TIMBER CREEK"
...
function customBlowerPressureControl(BLOWER,bl_pref,current_pressure,pressure_setpoint,zones_online,min_speed,max_speed)
  if autoBlowerSpeedControl(bl_pref,zones_online,min_speed,max_speed) == true then
    ...
    io[bl_pref..'speed'] = BLOWER['control']
    reg[bl_pref..'value'] = BLOWER['control']
  end
  if reg[bl_pref..'override'] == 1 then
    io[bl_pref..'run'] = reg[bl_pref..'control']
  else
    if reg[bl_pref..'control'] == 0 then
      stopBlower(bl_pref)
    else
      updateBlowerRunValue(bl_pref,reg[bl_pref..'control'])
    end
  end
end
