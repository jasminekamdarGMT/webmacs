...
function TestWebmacsScripts:test_update_blower_run_value()
  if blower_direction_control ~= true and has_blower_pressure_control ~= true then
    initSequence()
    for i, blower_id in pairs(blower_ids) do
      reg["blower"..blower_id.."cycle"] = 60 * 60
      io["blower"..blower_id.."run"] = 0
      updateBlowerRunValue('blower'..blower_id, 1)
      lu.assertEquals(io["blower"..blower_id.."run"], 1)
      reg["blower"..blower_id.."cycle"] = 60
      updateBlowerRunValue('blower'..blower_id, 1)
      lu.assertEquals(io["blower"..blower_id.."run"], 0)
      ...
      updateBlowerRunValue('blower'..blower_id, 1)
      updateBlowerRunValue('blower'..blower_id, 0)
      lu.assertEquals(io["blower"..blower_id.."run"], 0)
    end
  end
end
