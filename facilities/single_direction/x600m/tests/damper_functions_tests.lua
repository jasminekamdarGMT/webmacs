...
function TestWebmacsScripts:test_update_dampers()
  ...
  for i, zone_id in pairs(zone_ids) do
    ...
    updateDampers()
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 80)
    -- set zone online, turn off damper override
    -- set regime 2 temp setpoint to 125
    reg[zone_prefix..'control'] = 1
    reg[damper_prefix..'override'] = 0
    reg[zone_prefix..'regime'] = 2
    -- control temp is being compared to the current regime setpoint
    -- dampers should close as min temps are below regime 2 setpoint
    if #zone_probe_ids == 1 then
      if zone_probe_ids[1] == '' then
        reg[zone_prefix..'lvtemp'] = 100
      else
        reg[zone_prefix..'pAlvtemp'] = 100
      end
    elseif #zone_probe_ids == 2 then
      reg[zone_prefix..'pAlvtemp'] = 100
      reg[zone_prefix..'pBlvtemp'] = 145
    ...
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 15)
    lu.assertEquals(io[damper_prefix..'position'], 15)
    -- dampers should open as control temp is above regime 2 setpoint
    if #zone_probe_ids == 1 then
      if zone_probe_ids[1] == '' then
        reg[zone_prefix..'lvtemp'] = 145
      else
        reg[zone_prefix..'pAlvtemp'] = 145
      end
    elseif #zone_probe_ids == 2 then
      reg[zone_prefix..'pAlvtemp'] = 145
      reg[zone_prefix..'pBlvtemp'] = 150
    elseif #zone_probe_ids == 4 then
      reg[zone_prefix..'pAlvtemp'] = 145
      reg[zone_prefix..'pBlvtemp'] = 150
    ...
    updateDampers()
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 23.58)
    updateDampers()
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 29.58)
    updateDampers()
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 36.53)
    updateDampers()
    lu.assertEquals(_G['DAMPER_'..zone_id]['control'], 43.47)
    lu.assertEquals(io[damper_prefix..'position'], 43.47)
  end
end
