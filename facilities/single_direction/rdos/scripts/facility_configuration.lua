function initValues()
  ...
  ZONE_PROBE_IDS = {'A','B'}
  ZONE_LOG_COLUMNS = {'Temperature A', 'Temperature B', 'Blower Run', 'PFRP Time'}
...
function blowerLogValues(log_values,zn_id)
  for i,blower in ipairs(BLOWER_IDS) do
    if blower == zn_id then
      ...
      if ioAppearsValid("blower"..blower..'run') and io["blower"..blower..'run'] == 1 then
        blower_is_on = 'On'
      end
