...
function setBlowerSettings(){
	$("*").css("cursor", "progress");
	var values = ["PressureSetPoint", "MaxPressure", "MinPressure", "MinVFDSpeed", "BlowerGain", "BlowerIntegral", "BlowerDerivative"];
		...
		$.each(zones,function(k1, v1){
			$.each(data,function(k,v){
				...
				if(sections[3] == v1 + ".csv"){
					$('#' + v1 + 'date').html(sections[0].substring(5) + "/" + sections[1] + "/" + sections[2]);
					var x = v.name;
					return false;
				}
			});
...
function setValue(zone){
	...
	dialog = $( "#dialog-form" ).dialog({
		...
		buttons: {
			"Set Value": function() {
				if($('#value').val() != ""){
					io.set(zone + 'value', $('#value').val(), callback);
					$('#value').val("");
				}
				dialog.dialog('close');
			}
...
function setBlowerValue(blower){
	if(!($('#' + blower + 'override').is(':checked')))
		return;
	dialog = $( "#" + blower + "-form" ).dialog({
		autoOpen: false,
		height: 175,
		width: 250,
		modal: true,
		buttons: {
			"Set Value": function() {
				if($('#' + blower + 'value').val() != ""){
					io.set(blower + 'value', $('#' + blower + 'value').val(), callback);
					$('#' + blower + 'value').val("");
				}
				dialog.dialog('close');
			}
