function initValues()
  ...
  ZONE_PROBE_IDS = {'A', 'B'}
  BLWR_GRP_ZONE_IDS = {}
  BLWR_GRP_ZONE_IDS['01'] = uid(1,3)
  BLWR_GRP_ZONE_IDS['02'] = uid(4,6)
  BLWR_GRP_ZONE_IDS['03'] = uid(7,9)
  BLWR_GRP_ZONE_IDS['04'] = uid(10,12)
  ZONE_LOG_COLUMNS = {'Temperature A', 'Temperature B', 'Damper', 'Blower Speed', 'Duct Pressure', 'PFRP Time'}
...
function defaultSettings()
  local settings = {}
  settings["WirelessBaseStationIP"] = '192.168.12.15'
  settings["DataLoggingRate"] = "120"
  settings["FacilityName"] = "SALINAS"
  ...
  settings["Regime1ReferenceTemp"] = "Average"
  settings["Regime2ReferenceTemp"] = "Average"
  settings["Regime3ReferenceTemp"] = "Average"
...
function referenceTempForZone(zone_id)
  local ref_temp = 'Average'
  local regime = reg['zone'..zone_id..'regime']
  if regime ~= nil then
    ref_temp = SETTINGS['Regime'..regime..'ReferenceTemp']
  end
  return ref_temp
end
...
function updateDampers()
  for i,zn_id in ipairs(ZONE_IDS) do
    ...
    local ref_temp = referenceTempForZone(zn_id)
    if ref_temp == 'Lowest' then
      control_temp = minTempForZones({zn_id}, ZONE_PROBE_IDS)
    elseif ref_temp == 'Highest' then
      control_temp = maxTempForZones({zn_id}, ZONE_PROBE_IDS)
    end
...
function updateAlarms()
  ...
  blowerFaultAlarm(BLOWER_01, '01', 'Blower 1-3')
  blowerFaultAlarm(BLOWER_02, '02', 'Blower 4-6')
  blowerFaultAlarm(BLOWER_03, '03', 'Blower 7-9')
  blowerFaultAlarm(BLOWER_04, '04', 'Blower 10-12')
end
