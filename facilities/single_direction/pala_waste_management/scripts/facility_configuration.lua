...
function defaultSettings()
  ...
  settings["FacilityName"] = "Pala Waste Management"
...
function updateBlowers()
  if reg['estopactive'] == 1 then
    for i, blower_id in ipairs(BLOWER_IDS) do
      stopBlower("blower"..blower_id)
    end
    return
  end
...
function updateAlarms()
  local estop_active = 0
  for i,cntr_id in ipairs(CTNR_IDS) do
    if io["container"..cntr_id..'estop'] == 0 then
      estop_active = 1
      break
    end
  end
  reg['estopactive'] = estop_active
  for i,zn_id in ipairs(ZONE_IDS) do
