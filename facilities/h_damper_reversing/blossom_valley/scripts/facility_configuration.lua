...
function initValues()
  ...
  P_BL_IDS,S_BL_IDS,BLOWER_GROUP_ZONE_IDS = uid(1,6,'P'),uid(1,3,'S'),{}
...
function defaultSettings()
  ...
  settings["FacilityName"] = "BLOSSOM VALLEY"
...
function updateBlowers()
  ...
  for container, bl_grp in pairs(CTNR_BLWR_IDS) do
    if ioAppearsValid(str__container..container..'temp') and io[str__container..container..'temp'] >= tonumber(SETTINGS["MaxContainerTemp"]) then
      ctnr_over_stpnt = true
      for i, bl_id in pairs(bl_grp) do
        stopBlower(str__blower..bl_id)
      end
      local alarmage = reg[str__container..container..'alarmage']
      if (alarmage ~= nil and alarmage < 1) then
        sendAlarm('Container '..container, 'Blowers have shut down due to excessive container temp')
        reg[str__container..container..'alarmage'] = 3600000
      end
    else
      reg[str__container..container..'alarmage'] = 0
    end
  end
...
function updateDampers()
  for i,zn_id in ipairs(ZONE_IDS) do
    if zn_id ~= '10' and zn_id ~= '14' and zn_id ~= '24' then
      local dmpr_id,
            zones,
            avg_sp,
            zone_control =
      ...
      if dmpr_id == 'S1C' and zn_id == '09' then
        zones = {'09','10'}
      elseif dmpr_id == 'S2C' and zn_id == '13' then
        zones = {'13','14'}
      elseif dmpr_id == 'S3C' and zn_id == '23' then
        zones = {'23','24'}
      end
