...
function updateBlowerPIDPressureValues(pid,current_pressure,set_point,bl_pref,min_speed,max_speed)
  ...
  local curr_error = (set_point - current_pressure)
  ...
  if reg[bl_pref..'idletimer'] ~= nil and reg[bl_pref..'idletimer'] > 0 then
    ...
    if pid["prev_input"] == nil then
      diff = 0
    else
      diff = (current_pressure - pid["prev_input"]) / dt
    end
  ...
  pid["prev_input"] = current_pressure
end
...
function blowerPressureControl(BLOWER,bl_pref,current_pressure,pressure_setpoint,zones_online,min_speed,max_speed)
  ...
  if io[bl_pref..'run'] == 1 then
    if (reg[bl_pref..str__idletimer] ~= nil and reg[bl_pref..str__idletimer] < 1) or reg[bl_pref..str__idletimer] == nil then
      if io[bl_pref..str__speed] < min_speed then
        io[bl_pref..str__speed] = min_speed
      end
      if reg[bl_pref..'value'] < min_speed then
        reg[bl_pref..'value'] = min_speed
      end
    end
  end
end
...
function blowerDirectionControl(bl_pref,temp_avg,temp_sp,zones_online)
  ...
  elseif reg[bl_pref..'revtimer'] == 0 and zones_online > 0 then
    if reg[bl_pref..'direction'] == 1 then
      blowerSetDirection(bl_pref,0,half_of_idle_time * 2,SETTINGS["BlowerCycleNegativeTime"] * 60)
    else
      blowerSetDirection(bl_pref,1,half_of_idle_time * 2,SETTINGS["BlowerCyclePositiveTime"] * 60)
    end
  end
end
...
function blowerDirectionTempControl(bl_id,zones_online,top_avg,btm_avg,biofilters_temp_avg,top_sp,btm_sp)
  ...
  elseif zones_online > 0 then
    ...
    if biofilter_force_positive then
      ...
      if bl_dir == 0 then
        if btm_avg > btm_sp then
          new_dir = 1
        end
      else
        if top_avg > top_sp then
          new_dir = 0
        end
      end
    end
...
function misterValue(value)
  if SETTINGS['MisterRelayType'] ~= 'NC' then
    return value
  else
    if value > 0 then
      return 0
    ...
    if io[str__blower..bl_id..'run'] == 1 and reg[str__blower..bl_id..str__idletimer] == 0 then
      if reg[str__duct..bl_id..str__mister..'override'] == 0 then
        ...
        if temp >= tonumber(SETTINGS[str__Mister..bl_id..air_dir..'TempSetPoint']) then
          if timer == 0 then
            io[str__duct..bl_id..str__mister] = misterValue(1)
            reg[str__duct..bl_id..str__mistertimer] = ontime + offtime
          elseif timer <= offtime then
            io[str__duct..bl_id..str__mister] = misterValue(0)
          end
        else
          io[str__duct..bl_id..str__mister] = misterValue(0)
        end
